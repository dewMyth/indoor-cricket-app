...
export function oversAsFloatForRunRate(legalBalls: number, ballsPerOver: number): number {
...
export function isInningsOver(innings: InningsState, config: { totalOvers: number; ballsPerOver: number }, totalPlayers: number): boolean {
  ...
  const targetChased = innings.inningsNumber === 2 && innings.target !== undefined && innings.totalRuns >= innings.target;
...
export function computeBattingScore(stats: BattingStats): number {
  ...
  const strikeRateBonus = stats.ballsFaced >= 6 ? Math.max(0, (strikeRate - 100) / 10) : 0;
  return stats.runs * 1 + stats.fours * 1 + stats.sixes * 2 + strikeRateBonus;
}

export function computeBowlingScore(stats: BowlingStats, ballsPerOver: number): number {
...
export function calculateMatchAwards(match: MatchState): MatchAwards | undefined {
  ...
  const ensure = (playerId: string): PlayerAggregate => {
    ...
    if (!agg) {
      agg = { playerId, teamId: teamIdForPlayer(playerId), battingScore: 0, bowlingScore: 0, fieldingScore: 0 };
  ...
  const bestBatsman = [...list].sort((a, b) => b.battingScore - a.battingScore)[0];
  const bestBowler = [...list].sort((a, b) => b.bowlingScore - a.bowlingScore)[0];
  const bestFielder = [...list].sort((a, b) => b.fieldingScore - a.fieldingScore)[0];
  const mvp = [...list].sort(
    (a, b) => b.battingScore + b.bowlingScore + b.fieldingScore - (a.battingScore + a.bowlingScore + a.fieldingScore),
  )[0];

  return {
    mvp: { playerId: mvp.playerId, teamId: mvp.teamId, score: round1(mvp.battingScore + mvp.bowlingScore + mvp.fieldingScore) },
    bestBatsman: { playerId: bestBatsman.playerId, teamId: bestBatsman.teamId, score: round1(bestBatsman.battingScore) },
    bestBowler: { playerId: bestBowler.playerId, teamId: bestBowler.teamId, score: round1(bestBowler.bowlingScore) },
    bestFielder: { playerId: bestFielder.playerId, teamId: bestFielder.teamId, score: round1(bestFielder.fieldingScore) },
  };
}
...
export function findPlayerName(teamA: Team, teamB: Team, playerId?: string): string {
  if (!playerId) return '-';
  const player = [...teamA.players, ...teamB.players].find((p) => p.id === playerId);
  return player?.name ?? 'Unknown';
}

export function decideWinner(match: MatchState): { winnerTeamId?: string; margin?: string } {
  const { innings1, innings2 } = match;
  if (!innings1 || !innings2) return {};

  if (innings2.totalRuns > innings1.totalRuns) {
    const totalPlayersBatting2 = match.teamA.id === innings2.battingTeamId ? match.teamA.players.length : match.teamB.players.length;
