...
export default function Scoring() {
  ...
  const lastDismissedPlayerId = useAppSelector(
    (s) => s.match.lastDismissedPlayerId,
  );
  ...
  const handleRun = (runs: 0 | 1 | 2 | 3 | 4 | 5 | 6) => {
    dispatch(
      recordBall({
        outcome: (
          ["DOT", "RUN_1", "RUN_2", "RUN_3", "RUN_4", "RUN_5", "RUN_6"] as const
        )[runs],
      }),
    );
  };
  ...
  const battingLineupOutOrIn = Object.values(innings.battingStats)
    ...
    .map((b) => b.playerId)
    .concat(
      innings.strikerId ? [innings.strikerId] : [],
      innings.nonStrikerId ? [innings.nonStrikerId] : [],
    );
  ...
  return (
    <div className="min-h-screen bg-night-800 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <BallTimeline
          history={innings.ballHistory}
          ballsPerOver={match.config.ballsPerOver}
        />
      </div>
      ...
      {pendingNewBowler && !pendingNewBatsman && (
        <NewBowlerModal
          ...
          onConfirm={(playerId) =>
            dispatch(selectBowler({ bowlerId: playerId }))
          }
        />
      )}
